import { Switch, Stack, Typography } from '@mui/material';
import { styled } from '@mui/material/styles';
import React from 'react';
import { fontColor } from '../helper/utils';
import { ColorModeContext } from './RootLayout';

const ViewToggle = (props: { isCardView: boolean; handleToggle: () => void }) => {
    const { isCardView, handleToggle } = props;
    const colorMode = React.useContext(ColorModeContext);

    const AntSwitch = styled(Switch)(({ theme }) => ({
        width: 28,
        height: 16,
        padding: 0,
        display: 'flex',
        '&:active': {
            '& .MuiSwitch-thumb': {
                width: 15,
            },
            '& .MuiSwitch-switchBase.Mui-checked': {
                transform: 'translateX(9px)',
            },
        },
        '& .MuiSwitch-switchBase': {
            padding: 2,
            '&.Mui-checked': {
                transform: 'translateX(12px)',
                color: '#fff',
                '& + .MuiSwitch-track': {
                    opacity: 1,
                    backgroundColor: theme.palette.mode === 'dark' ? 'rgba(255,255,255,.35)' : 'rgba(0,0,0,.25)',
                },
            },
        },
        '& .MuiSwitch-thumb': {
            boxShadow: '0 2px 4px 0 rgb(0 35 11 / 20%)',
            width: 12,
            height: 12,
            borderRadius: 6,
            transition: theme.transitions.create(['width'], {
                duration: 200,
            }),
        },
        '& .MuiSwitch-track': {
            borderRadius: 16 / 2,
            opacity: 1,
            backgroundColor: theme.palette.mode === 'dark' ? 'rgba(255,255,255,.35)' : 'rgba(0,0,0,.25)',
            // backgroundColor: 'red',
            boxSizing: 'border-box',
        },
    }));

    return (
        <>
            <Stack direction="row" spacing={1} alignItems="center">
                <Typography color={fontColor(colorMode.mode)}>Table</Typography>
                <AntSwitch onChange={handleToggle} checked={isCardView} inputProps={{ 'aria-label': 'ant design' }} />
                <Typography color={fontColor(colorMode.mode)}>Card</Typography>
            </Stack>
        </>
    );
};

export default ViewToggle;
